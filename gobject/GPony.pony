...
use @g_object_new[GPonyStruct](gtype: U64, first: Pointer[U8] tag, ...)
...
class GPony is GObjectInterface
  ...
  new create() =>
    ...
    if (gtype == 0) then
      let gtypeinfo: GTypeInfoStruct = GTypeInfoStruct
      gtypeinfo.class_size = 1088 // No methods, so same as GObject
      gtypeinfo.instance_size = 256
      gtypeinfo.n_preallocs = 0

      gtype = @g_type_register_static(80, "GPony".cstring(), NullablePointer[GTypeInfoStruct](gtypeinfo), 0)
    end
  ...
  fun ref set_pointer[B: Any](data': B) => None
    ptr.data = GValueStruct
    @g_value_init(NullablePointer[GValueStruct](ptr.data), U64(68))
    @g_value_set_pointer(NullablePointer[GValueStruct](ptr.data), data')

  fun ref get_pointer[C: Any](): C =>
    @g_value_get_pointer[C](NullablePointer[GValueStruct](ptr.data))

  fun ref get_ptr(): GObjectStruct => ptr.parent_instance
